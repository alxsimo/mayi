 ...
 *
 * This example will run the runnable returned by onPermissionGranted() if the
 * WRITE_EXTERNAL_STORAGE permission is
 * already granted. Otherwise it will prompt the user and run the runnable returned by
 * onPermissionGranted() or
 * onPermissionDenied() depending on whether the user accepted or not. If onUiThread() is specified
 * then all callbacks
...
public class Mayi {
   ...
   *
   * Note: The provided context needs to be an Activity context in order to prompt. Use
   * doNotPrompt()
  ...
  /**
   * This method will block until the specified permissions have been granted or denied by the
   * user.
   * If needed the user will be prompted.
   *
   * @return true if all of the permissions have been granted. False if any of the permissions have
   * been denied.
   */
  ...
  /**
   * Callback for Activity.onRequestPermissionsResult(). All activities that prompt for permissions
   * using this class
  ...
  private static synchronized void processGrantResults(@NonNull String[] permissions,
  ...
  private static synchronized void processQueue(Activity activity, String[] permissions,
      int[] grantResults) {
  ...
  private static HashSet<String> collectGrantedPermissions(@NonNull String[] permissions,
      @NonNull int[] grantResults) {
    return filterPermissionsByResult(permissions, grantResults, PackageManager.PERMISSION_GRANTED);
  }

  private static HashSet<String> collectDeniedPermissions(@NonNull String[] permissions,
      @NonNull int[] grantResults) {
    return filterPermissionsByResult(permissions, grantResults, PackageManager.PERMISSION_DENIED);
  }

  private static HashSet<String> filterPermissionsByResult(@NonNull String[] permissions,
      @NonNull int[] grantResults, int result) {
  ...
  private static boolean allPermissionsGranted(PermissionBlock block,
  ...
  private static boolean atLeastOnePermissionDenied(PermissionBlock block,
